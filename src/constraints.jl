...
function _optimization_info(network::Network, tspan::Tuple)
     ...
     for (ix, node) in enumerate(values(get_nodes(network)))
         ...
         for (jx, organism) in enumerate(get_organisms(node))
             ...
             optinfo_dict[ix]["organism"][jx]["homozygous_modified"] = get_homozygous_modified(node, organism)
             ...
             optinfo_dict[ix]["organism"][jx]["stage_density"] = Dict()
              ...
              for (key_lifestage, life_stage) in get_lifestages(node, organism)
                  ...
                  optinfo_dict[ix]["organism"][jx]["substage_count"][key_lifestage] = substage_count
                  optinfo_dict[ix]["organism"][jx]["total_stage_count"] += substage_count
                  optinfo_dict[ix]["organism"][jx]["stage_temperature_response"] = Dict()
                  optinfo_dict[ix]["organism"][jx]["stage_density"][key_lifestage] = get_density(node, organism, key_lifestage)

                  for lx in 1:count_genotypes(network, get_name(node), organism)
                      response = [temperature_effect(temp, life_stage) for temp in node.temperature.values]
                      ...
                      optinfo_dict[ix]["organism"][jx]["stage_temperature_response"] = spatialtempresponse_dict
                  end
              end
          end
      end
...
function _add_constraint(model::JuMP.Model, optinfo_dict::Dict, 
    life_stage::Type{<:Male}, node_strategy::Dict, homozygous_modified, wildtype)
...
function _add_constraint(model::JuMP.Model, optinfo_dict::Dict, 
    life_stage::Type{<:Female}, node_strategy::Dict, homozygous_modified, wildtype)
    ...
    control_F[n,o,wildtype,homozygous_modified,t] <= data[n]["org_con"][o]["stage_con"][life_stage][g][t])

    # MAXIMUM
    JuMP.@constraint(model,
    control_limit_total,
    sum(control_F) <= max_overall)
...
function _calculate_release_constraints(network::Network, tspan::Tuple,
    ...
    for (ix, node) in enumerate(values(nodes))
        ...
        for (jx, organism) in enumerate(get_organisms(node))
            ...
            for (key_lifestage, life_stage) in get_lifestages(node, organism)
                releaseconstraints_dict[ix]["org_con"][jx]["stage_con"][key_lifestage] = Dict()
                for lx in 1:count_genotypes(network, get_name(node), organism)
                    releaseconstraints_dict[ix]["org_con"][jx]["stage_con"][key_lifestage][lx] = zeros(length(tspan[1]:tspan[2]))
                end
            end
        end
    end
    ...
    for (node_number, strategy) in node_strategy
        ...
        if all(isequal(node_strategy[1].release_max_over_timehorizon, strategy.release_max_over_timehorizon)) == false
        ...
        strategy.release_start_time === nothing ? (strategy.release_start_time = tspan[1]) : strategy.release_start_time
        strategy.release_end_time === nothing ? (strategy.release_end_time = tspan[2]) : strategy.release_end_time
    end

    for (node_number, strategy) in node_strategy

        # TODO: fix
        for tx in strategy.release_start_time:strategy.release_time_interval:strategy.release_end_time
           releaseconstraints_dict[node_number]["org_con"][1]["stage_con"][strategy.release_this_life_stage][strategy.release_this_gene_index][tx] = strategy.release_size_max_per_timestep
        end
    end

    optinfo_dict["constraints"] = releaseconstraints_dict

    _add_constraint(model, optinfo_dict, node_strategy[1].release_this_life_stage, node_strategy, 
    homozygous_modified, wildtype)
