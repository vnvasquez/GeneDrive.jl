@testset "Temp Response Functions" begin
    ...
    test_eggmortalitymoustaid = EggMortalityMoustaid(30.33, 9.395, 3.6576, 110.0014, 0.00077)
    ...
    test_larvamortalitymoustaid = LarvaMortalityMoustaid(36.79, 2.354, 2.01938, 73.1928, 0.00046)
    ...
    test_pupamortalitymoustaid = PupaMortalityMoustaid(37.4679, 9.96278, 1.0, 38.0, 0.03487954)
    ...
    test_eggdurationabiodun = EggDurationAbiodun(6.0, 1.011, 20.212, 1.0, 12.096, 4.839, 1.0)
    response = get_temperature_response(27.0,test_eggdurationabiodun)
    @test isapprox(response, 1.166976; atol = 1e-3)

    test_eggmortalityabiodun = EggMortalityAbiodun(6.0, 1.011, 20.212, 1.0, 12.096, 4.839, 1.0)
