@testset "Dynamic Model" begin
    ...
    test_release = Release(test_node, AedesAegypti, Male, test_release_gene,
          test_release_times, test_release_size);
    test_shocks = TemperatureShockData(test_node,[(20.0, 30.0)],[2.0])
    test_sol = solve_dynamic_model(test_node,[test_release],test_shocks,test_solver,test_tspan)
    ...
    move_rate = 0.002
    test_migration = Dict(
        ("Male", "WW") => Dict((:TestNode, :TestNode2) => move_rate,
                            (:TestNode2, :TestNode) => move_rate),
        ("Male", "ww") => Dict((:TestNode, :TestNode2) => move_rate,
                            (:TestNode2, :TestNode) => move_rate),
        ("Female", "WW") => Dict((:TestNode, :TestNode2) => move_rate,
                            (:TestNode2, :TestNode) => move_rate),
        ("Female", "ww") => Dict((:TestNode, :TestNode2) => move_rate,
                            (:TestNode2, :TestNode) => move_rate),
        )
